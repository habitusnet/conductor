...
import { createConductorServer, type ConductorServerOptions } from './server.js';
...
const registeredTools: Map<string, { description: string; schema: any; handler: Function }> =
  new Map();
const registeredResources: Map<string, { description: string; handler: Function }> = new Map();

const mockMcpServer = {
  tool: vi.fn((name: string, description: string, schema: any, handler: Function) => {
    registeredTools.set(name, { description, schema, handler });
  }),
  resource: vi.fn((uri: string, description: string, handler: Function) => {
...
describe('MCP Server', () => {
  ...
  describe('createConductorServer', () => {
    ...
    it('should register project status resource', () => {
      ...
      expect(registeredResources.has(`conductor://project/${projectId}/status`)).toBe(true);
    });
  });
  ...
  describe('Task Tools', () => {
    ...
    describe('conductor_list_tasks', () => {
      it('should list tasks with filters', async () => {
        ...
        const result = await tool.handler({ status: 'pending', priority: 'high' });
    ...
    describe('conductor_claim_task', () => {
      it('should claim task successfully', async () => {
        ...
        expect(mockStateStore.claimTask).toHaveBeenCalledWith('task-1', 'claude-123');
    ...
    describe('conductor_update_task', () => {
      it('should update task status', async () => {
        ...
        expect(mockStateStore.updateTask).toHaveBeenCalledWith('task-1', { status: 'completed' });
      ...
      it('should update task with notes', async () => {
        ...
        mockStateStore.updateTask.mockReturnValue({ ...currentTask, status: 'in_progress' });
      ...
      it('should update task with tokens and blocked info', async () => {
        mockStateStore.updateTask.mockReturnValue({ id: 'task-1', status: 'blocked' });
  ...
  describe('Agent Tools', () => {
    ...
    describe('conductor_heartbeat', () => {
      ...
      it('should update status with heartbeat', async () => {
        ...
        expect(mockStateStore.heartbeat).toHaveBeenCalledWith('claude-123');
        expect(mockStateStore.updateAgentStatus).toHaveBeenCalledWith('claude-123', 'working');
  ...
  describe('Access Request Tools', () => {
    ...
    describe('conductor_request_access', () => {
      ...
      it('should create access request when auto-approved', async () => {
        ...
        expect(mockStateStore.createAccessRequest).toHaveBeenCalledWith(projectId, {
          agentId: 'claude-123',
          agentName: 'Claude',
          agentType: 'claude',
          capabilities: ['typescript'],
  ...
  describe('Context Management Tools', () => {
    ...
    describe('conductor_refresh_context', () => {
      it('should return full context refresh', async () => {
        const mockContext = {
          ...
          deniedPaths: ['secrets/'],
          relevantPatterns: [
            { file: 'src/utils.ts', lineRange: '10-20', description: 'Helper functions' },
          ],
        };
    ...
    describe('conductor_get_onboarding_config', () => {
      ...
      it('should return message when no config found', async () => {
        ...
        expect(result.content[0].text).toContain('No onboarding configuration found');
      });
  ...
  describe('Resources', () => {
    ...
    describe('project status resource', () => {
      it('should return project status', async () => {
        ...
        const resource = registeredResources.get(`conductor://project/${projectId}/status`)!;
      ...
      it('should handle project without budget', async () => {
        mockStateStore.getProject.mockReturnValue({ id: projectId, name: 'Test' });
        mockStateStore.listTasks.mockReturnValue([]);

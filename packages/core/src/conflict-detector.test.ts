...
import { ConflictDetector, ZoneManager, type FileZone } from './conflict-detector.js';
...
describe('ConflictDetector', () => {
  ...
  describe('detectOverlappingTasks', () => {
    it('should detect tasks working on the same file', async () => {
      ...
      const conflicts = await detector.detectOverlappingTasks(tasks, 'project-1');
  ...
  describe('isFileSafeToModify', () => {
    it('should return safe for new files (no git history)', async () => {
      ...
      const result = await detector.isFileSafeToModify('src/new-file.ts', 'Claude');
    ...
    it('should return safe when different agent modified long ago', async () => {
      ...
      const result = await detector.isFileSafeToModify('src/auth.ts', 'Claude', 5);
      expect(result.safe).toBe(true);
    });

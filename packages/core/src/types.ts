...
export type AgentId = 'claude' | 'gemini' | 'codex' | 'gpt4' | 'llama' | (string & {});
export type AgentStatus = 'idle' | 'working' | 'blocked' | 'offline';
export type AgentProvider = 'anthropic' | 'google' | 'openai' | 'meta' | 'custom';
...
export const ProjectAgentSchema = z.object({
  ...
  agentId: z.string(),
  role: z.enum(['lead', 'contributor', 'reviewer', 'observer']).default('contributor'),
...
export const TaskSchema = z.object({
  ...
  title: z.string(),
  description: z.string().optional(),
  status: z.enum(['pending', 'claimed', 'in_progress', 'completed', 'failed', 'blocked', 'cancelled']).default('pending'),

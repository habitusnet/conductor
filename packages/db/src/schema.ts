...
export const agents = sqliteTable('agents', {
  ...
  name: text('name').notNull(),
  provider: text('provider', { enum: ['anthropic', 'google', 'openai', 'meta', 'custom'] }).notNull(),
  ...
  status: text('status', { enum: ['idle', 'working', 'blocked', 'offline'] }).default('idle'),
...
export const projects = sqliteTable('projects', {
  ...
  conflictStrategy: text('conflict_strategy', { enum: ['lock', 'merge', 'zone', 'review'] }).default('lock'),
...
export const projectAgents = sqliteTable('project_agents', {
  ...
  role: text('role', { enum: ['lead', 'contributor', 'reviewer', 'observer'] }).default('contributor'),
...
export const tasks = sqliteTable('tasks', {
  ...
  description: text('description'),
  status: text('status', { enum: ['pending', 'claimed', 'in_progress', 'completed', 'failed', 'blocked', 'cancelled'] }).default('pending'),
...
export const taskActivities = sqliteTable('task_activities', {
  ...
  action: text('action', { enum: ['created', 'claimed', 'started', 'progress_update', 'completed', 'failed', 'blocked', 'unblocked', 'reassigned', 'cancelled', 'commented'] }).notNull(),
  description: text('description'),
  metadata: text('metadata').default('{}'),
  createdAt: text('created_at').default('CURRENT_TIMESTAMP'),
});
...
export const fileConflicts = sqliteTable('file_conflicts', {
  ...
  strategy: text('strategy', { enum: ['lock', 'merge', 'zone', 'review'] }).notNull(),
  resolution: text('resolution', { enum: ['accepted', 'rejected', 'merged', 'waiting'] }),
...
export const connectorConfigs = sqliteTable('connector_configs', {
  id: text('id').primaryKey(),
  organizationId: text('organization_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  type: text('type', { enum: ['github', 'gitlab', 'slack', 'discord', 'webhook'] }).notNull(),
  name: text('name').notNull(),

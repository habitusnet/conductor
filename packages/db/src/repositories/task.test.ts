...
describe('TaskRepository', () => {
  ...
  describe('findByProject', () => {
    ...
    it('should filter by multiple statuses', async () => {
      ...
      await repo.findByProject('proj-123', { status: ['pending', 'in_progress'] });
  ...
  describe('claim', () => {
    it('should claim pending task', async () => {
      const claimedTask = { id: 'task-123', status: 'claimed', assignedTo: 'agent-123' };
  ...
  describe('start', () => {
    it('should start claimed task', async () => {
      const startedTask = { id: 'task-123', status: 'in_progress', assignedTo: 'agent-123' };
  ...
  describe('complete', () => {
    it('should complete task with result', async () => {
      const completedTask = { id: 'task-123', status: 'completed', assignedTo: 'agent-123' };
      mockDb.returning.mockResolvedValue([completedTask]);

      const result = await repo.complete('task-123', 'Task completed successfully', 5000);
  ...
  describe('fail', () => {
    it('should mark task as failed with error', async () => {
      const failedTask = { id: 'task-123', status: 'failed', assignedTo: 'agent-123' };

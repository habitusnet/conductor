...
export class OpenAIConnector implements Connector {
  ...
  async analyzeTask(taskDescription: string, projectContext: string) {
    return this.sendMessage([
      {
        role: 'system',
        content: 'You are a task analysis assistant. Analyze the given task and provide: 1) Estimated complexity (low/medium/high), 2) Estimated tokens needed, 3) Required capabilities, 4) Potential risks or blockers.',
      },
      {
        role: 'user',

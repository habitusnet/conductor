...
import { createConnector, GitHubConnector, ClaudeConnector, GeminiConnector, OpenAIConnector, WebhookConnector } from './index';

// Mock external dependencies used by connectors
vi.mock('@octokit/rest', () => ({
  Octokit: vi.fn().mockImplementation(() => ({
    users: { getAuthenticated: vi.fn().mockResolvedValue({ data: { login: 'test' } }) },
    repos: { get: vi.fn(), listBranches: vi.fn(), getContent: vi.fn(), createOrUpdateFileContents: vi.fn() },
    issues: { create: vi.fn() },
    pulls: { create: vi.fn() },
  })),
}));
...
describe('createConnector factory', () => {
  ...
  describe('openai connector', () => {
    ...
    it('should create connector with organization', () => {
      const config = { apiKey: 'my-key', model: 'gpt-4o', organization: 'org-123' };
      const connector = createConnector('openai', config);
  ...
  describe('unknown connector type', () => {
    it('should throw error for unknown type', () => {
      expect(() => createConnector('unknown', {})).toThrow('Unknown connector type: unknown');
    });
    ...
    it('should throw error for invalid type', () => {
      expect(() => createConnector('invalid', {})).toThrow('Unknown connector type: invalid');
    });
  });

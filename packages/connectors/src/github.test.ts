...
describe('GitHubConnector', () => {
  ...
  describe('getRepo', () => {
    it('should throw if not connected', async () => {
      await expect(connector.getRepo('owner', 'repo')).rejects.toThrow('Not connected');
    });
    ...
    it('should return repo data when connected', async () => {
      ...
      const mockRepoData = { id: 1, name: 'test-repo', full_name: 'owner/test-repo' };
      mockOctokit.repos.get.mockResolvedValue({ data: mockRepoData });

      const result = await connector.getRepo('owner', 'test-repo');
      expect(result).toEqual(mockRepoData);
      expect(mockOctokit.repos.get).toHaveBeenCalledWith({ owner: 'owner', repo: 'test-repo' });
    });
  });

  describe('listBranches', () => {
    it('should throw if not connected', async () => {
      await expect(connector.listBranches('owner', 'repo')).rejects.toThrow('Not connected');
    });
    ...
    it('should return branches when connected', async () => {
      ...
      expect(mockOctokit.repos.listBranches).toHaveBeenCalledWith({ owner: 'owner', repo: 'repo' });
    });
  });

  describe('createIssue', () => {
    it('should throw if not connected', async () => {
      await expect(connector.createIssue('owner', 'repo', 'title', 'body')).rejects.toThrow('Not connected');
    });
    ...
    it('should create issue when connected', async () => {
      ...
      const result = await connector.createIssue('owner', 'repo', 'Test Issue', 'Issue body', ['bug']);
  ...
  describe('createPullRequest', () => {
    it('should throw if not connected', async () => {
      await expect(
        connector.createPullRequest('owner', 'repo', 'title', 'body', 'feature', 'main')
    ...
    it('should create pull request when connected', async () => {
      ...
      const mockPR = { id: 1, number: 99, title: 'Test PR', html_url: 'https://github.com/owner/repo/pull/99' };
      mockOctokit.pulls.create.mockResolvedValue({ data: mockPR });

      const result = await connector.createPullRequest('owner', 'repo', 'Test PR', 'PR body', 'feature', 'main');
  ...
  describe('getContent', () => {
    it('should throw if not connected', async () => {
      await expect(connector.getContent('owner', 'repo', 'path')).rejects.toThrow('Not connected');
    });
    ...
    it('should get content with ref', async () => {
      ...
      await connector.getContent('owner', 'repo', 'README.md', 'feature-branch');
      expect(mockOctokit.repos.getContent).toHaveBeenCalledWith({
        owner: 'owner',
        repo: 'repo',
        path: 'README.md',
  ...
  describe('createOrUpdateFile', () => {
    it('should throw if not connected', async () => {
      await expect(
        connector.createOrUpdateFile('owner', 'repo', 'path', 'content', 'message')
      ).rejects.toThrow('Not connected');
    });
    ...
    it('should create new file when connected', async () => {
      ...
      const mockResult = { content: { sha: 'abc123' }, commit: { sha: 'def456' } };
      mockOctokit.repos.createOrUpdateFileContents.mockResolvedValue({ data: mockResult });
  ...
  describe('syncInstructionFile', () => {
    it('should return file content when found', async () => {
      ...
      const result = await connector.syncInstructionFile('owner', 'repo', 'CLAUDE.md');
    ...
    it('should return null when content field is missing', async () => {
      ...
      const result = await connector.syncInstructionFile('owner', 'repo', 'folder');
      expect(result).toBeNull();
    });
  });
});

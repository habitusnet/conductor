...
describe('GeminiConnector', () => {
  ...
  describe('sendMessage', () => {
    it('should throw if not connected', async () => {
      await expect(connector.sendMessage('Hello')).rejects.toThrow('Not connected');
    });
  ...
  describe('generateCode', () => {
    ...
    it('should generate code for the given prompt and language', async () => {
      ...
      mockGenerateContent.mockResolvedValue({
        response: {
          text: () => 'function sort(arr: number[]): number[] { return arr.sort((a, b) => a - b); }',
        },
      });

      const result = await connector.generateCode('Sort an array', 'TypeScript');

...
import { getFirestoreStateStore, getProjectId, FirestoreStateStore } from './firebase-db';
...
export async function setOnboardingConfig(ctx: FirebaseApiContext, config: any) {
...
export async function denyAccessRequest(
  ...
  await ctx.store.denyAccessRequest(ctx.projectId, requestId, reviewedBy, reason);
...
export async function expireOldRequests(
  ...
  // TODO: Implement with Firestore batch operations
  return 0;
}

...
export class D1StateStore {
  ...
  async listAgents(projectId: string): Promise<AgentProfile[]> {
    ...
    return (result.results || []).map((row) => this.rowToAgent(row));
  }
  ...
  async listAccessRequests(projectId: string, filters?: AccessRequestFilters): Promise<AccessRequest[]> {
    ...
    const stmt = this.db.prepare(query);
    const result = await stmt.bind(...params).all();
    return (result.results || []).map((row) => this.rowToAccessRequest(row));
  }

  async approveAccessRequest(requestId: string, reviewedBy: string): Promise<void> {
  ...
  async denyAccessRequest(requestId: string, reviewedBy: string, reason?: string): Promise<void> {
    const now = new Date().toISOString();
  ...
  async getOnboardingConfig(projectId: string): Promise<OnboardingConfig | null> {
    const row = await this.db
  ...
  async setOnboardingConfig(projectId: string, config: Partial<OnboardingConfig>): Promise<void> {
  ...
  async getProjectSpend(projectId: string): Promise<number> {
    const result = await this.db
      .prepare('SELECT SUM(cost) as total FROM cost_events WHERE project_id = ?')
      .bind(projectId)
      .first();
  ...
  async getCostEvents(projectId: string): Promise<CostEvent[]> {
    const result = await this.db
      .prepare('SELECT * FROM cost_events WHERE project_id = ? ORDER BY created_at DESC LIMIT 100')
      .bind(projectId)
      .all();
  ...
  async resolveConflict(projectId: string, conflictId: string, resolution: string): Promise<void> {
    await this.db
      .prepare('UPDATE conflicts SET resolved_at = datetime("now"), resolution = ? WHERE id = ? AND project_id = ?')
      .bind(resolution, conflictId, projectId)
      .run();
  }

  async releaseLock(projectId: string, filePath: string, agentId: string): Promise<void> {
    await this.db
      .prepare('DELETE FROM file_locks WHERE project_id = ? AND file_path = ? AND agent_id = ?')
  ...
  async updateAgentStatus(agentId: string, status: string): Promise<void> {
    await this.db
      .prepare("UPDATE agents SET status = ?, last_heartbeat = datetime('now') WHERE id = ?")
      .bind(status, agentId)
      .run();
  }
  ...
  private rowToProject(row: Record<string, unknown>): Project {
    return {
      ...
      conflictStrategy: (row.conflict_strategy as Project['conflictStrategy']) || 'lock',
  ...
  private rowToAccessRequest(row: Record<string, unknown>): AccessRequest {
    return {
      ...
      capabilities: JSON.parse((row.capabilities as string) || '[]'),
      requestedRole: (row.requested_role as AccessRequest['requestedRole']) || 'contributor',
...
export type { Project, Task, AgentProfile, AccessRequest, OnboardingConfig, CostEvent, TaskFilters, AccessRequestFilters };

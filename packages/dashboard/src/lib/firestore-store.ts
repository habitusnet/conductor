...
export class FirestoreStateStore {
  ...
  private onboardingRef(projectId: string): DocumentReference {
    return this.projectsRef().doc(projectId).collection('onboarding').doc('config');
  }
  ...
  async updateTask(projectId: string, taskId: string, updates: Partial<Task>): Promise<void> {
    ...
    if (updates.blockedBy !== undefined) updateData.blockedBy = updates.blockedBy;
  ...
  async getAgent(agentId: string, projectId: string): Promise<AgentProfile | null> {
    const doc = await this.agentsRef(projectId).doc(agentId).get();
    if (!doc.exists) return null;
    return this.docToAgent(doc);
  }

  async updateAgentStatus(projectId: string, agentId: string, status: string): Promise<void> {
  ...
  async approveAccessRequest(projectId: string, requestId: string, reviewedBy: string): Promise<void> {
    const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days

    await this.accessRequestsRef(projectId).doc(requestId).update({
      status: 'approved',
  ...
  async denyAccessRequest(projectId: string, requestId: string, reviewedBy: string, reason?: string): Promise<void> {
    await this.accessRequestsRef(projectId).doc(requestId).update({
      status: 'denied',
      reviewedAt: FieldValue.serverTimestamp(),
      reviewedBy,
  ...
  async getActiveLocks(projectId: string): Promise<FileLock[]> {
    ...
    return snapshot.docs.map((doc) => {
      ...
      return {
        filePath: data.filePath,
        agentId: data.agentId,
        lockedAt: toDate(data.lockedAt)?.toISOString() || new Date().toISOString(),
        expiresAt: toDate(data.expiresAt)?.toISOString() || new Date().toISOString(),
      };
    });
  }
  ...
  private docToAccessRequest(doc: FirebaseFirestore.DocumentSnapshot): AccessRequest {
    const data = doc.data()!;
    return {
      id: doc.id,
      projectId: data.projectId,

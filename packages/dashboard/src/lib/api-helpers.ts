 ...
 */
export async function approveAccessRequest(ctx: ApiContext, requestId: string, reviewedBy: string) {
  if (ctx.isD1) {
    const store = ctx.store as D1StateStore;
 ...
 */
export async function denyAccessRequest(ctx: ApiContext, requestId: string, reviewedBy: string, reason?: string) {
  if (ctx.isD1) {
    const store = ctx.store as D1StateStore;
 ...
 */
export async function expireOldRequests(ctx: ApiContext, olderThanHours: number) {
  if (ctx.isD1) {
 ...
 */
export async function resolveConflict(ctx: ApiContext, conflictId: string, resolution: string) {
  if (ctx.isD1) {
    const store = ctx.store as D1StateStore;
 ...
 */
export async function releaseLock(ctx: ApiContext, filePath: string, agentId: string) {
  if (ctx.isD1) {
    const store = ctx.store as D1StateStore;
 ...
 */
export async function updateTask(ctx: ApiContext, taskId: string, updates: any) {
  if (ctx.isD1) {
    const store = ctx.store as D1StateStore;
 ...
 */
export async function updateAgentStatus(ctx: ApiContext, agentId: string, status: 'idle' | 'working' | 'blocked' | 'offline') {
  if (ctx.isD1) {
    const store = ctx.store as D1StateStore;

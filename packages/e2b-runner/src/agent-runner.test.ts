...
import { AgentRunner, createClaudeCodeRunner, createAiderRunner } from './agent-runner.js';
import type { SandboxEvent, AgentRunnerConfig } from './types.js';
...
describe('AgentRunner', () => {
  ...
  let commandResults: Map<string, { stdout: string; stderr: string; exitCode: number }>;
  ...
  beforeEach(() => {
    ...
    commandResults.set('default', { stdout: 'success', stderr: '', exitCode: 0 });
      ...
      commands: {
        run: vi.fn().mockImplementation((cmd: string) => {
          ...
          if (cmd.includes('git clone')) {
            return Promise.resolve(commandResults.get('git') || { stdout: 'Cloning...', stderr: '', exitCode: 0 });
          }
          if (cmd.includes('claude')) {
            return Promise.resolve(commandResults.get('claude') || { stdout: 'Agent completed', stderr: '', exitCode: 0 });
          }
          if (cmd.includes('aider')) {
            return Promise.resolve(commandResults.get('aider') || { stdout: 'Aider completed', stderr: '', exitCode: 0 });
          }
  ...
  describe('startAgent', () => {
    ...
    it('should cleanup sandbox on setup failure', async () => {
      mockSandbox.commands.run.mockRejectedValueOnce(new Error('Git clone failed'));
      ...
      await expect(runner.startAgent(config)).rejects.toThrow('Git clone failed');
  ...
  describe('runAgent', () => {
    it('should run agent and return success result', async () => {
      commandResults.set('claude', { stdout: 'Task completed', stderr: '', exitCode: 0 });
    ...
    it('should return failure result on non-zero exit', async () => {
      commandResults.set('claude', { stdout: '', stderr: 'Error occurred', exitCode: 1 });
    ...
    it('should handle exceptions and return error result', async () => {
      vi.mocked(Sandbox.create).mockRejectedValueOnce(new Error('Sandbox creation failed'));
  ...
  describe('stopAllAgents', () => {
    it('should stop all running agents', async () => {
      const killFns = [vi.fn().mockResolvedValue(undefined), vi.fn().mockResolvedValue(undefined)];
...
describe('createClaudeCodeRunner', () => {
  ...
  it('should create a pre-configured Claude Code runner', () => {
    const { runner, startAgent, runAgent } = createClaudeCodeRunner('http://localhost:3001');
  ...
  it('should start agent with correct config', async () => {
    ...
    const instance = await startAgent('agent-1', 'project-1', 'https://github.com/org/repo');
...
describe('createAiderRunner', () => {
  ...
  it('should start agent with correct config', async () => {
    ...
    const instance = await startAgent('aider-1', 'project-1', 'https://github.com/org/repo');
